...
def extract_features(model, data_loader, device, num_batches=None):
    ...
    with torch.no_grad():
        for batch_idx, (ecg_signals, labels) in enumerate(tqdm(data_loader, desc="Extracting features")):
...
# =========================
# QMIA Attacker (Paper-Aligned)
# =========================

class QMIAAttacker:
    """
    Quantile-based Membership Inference Attack
    Correct implementation aligned with the paper.
    """
    ...
    def __init__(self, config, num_classes):
        ...
        # Log-spaced quantiles focused on tail
        ...
        print(f"[QMIA] Using {config.num_quantiles} quantiles")
    ...
    # ==========================================================
    # TRAIN ONLY ON NON-MEMBER DATA
    # ==========================================================

    def train_attack_model(self, attack_model, nonmember_logits, nonmember_labels):

        print("\n[QMIA] Training quantile regressor on NON-MEMBER data...")

        nonmember_logits = nonmember_logits.to(self.device)
        nonmember_labels = nonmember_labels.to(self.device)

        train_loader = DataLoader(
            TensorDataset(nonmember_logits, nonmember_labels),
        ...
        for epoch in range(self.config.attack_epochs):
            ...
            for batch_logits, batch_labels in train_loader:
                ...
                scores = self.compute_hinge_scores(batch_logits, batch_labels)
    ...
    # ==========================================================
    # CDF-BASED MEMBERSHIP SCORE
    # ==========================================================

    def compute_membership_score(self, attack_model, logits, labels):

        logits = logits.to(self.device)
        labels = labels.to(self.device)

        with torch.no_grad():
            scores = self.compute_hinge_scores(logits, labels)
            predicted_quantiles = attack_model(logits)

        # Estimate CDF position:
        # fraction of predicted quantiles <= observed score
        indicators = (predicted_quantiles <= scores.unsqueeze(1)).float()
        estimated_cdf = indicators.mean(dim=1)

        # estimated_cdf = fraction of quantiles ≤ observed score → essentially CDF of non-members at this hinge score


        # Tail probability
        membership_score = estimated_cdf

        '''
        If the observed hinge score is higher than most non-member quantiles, estimated_cdf ~ 1 → membership_score ~ 0 → unlikely member

        If the observed hinge score is lower than most non-member quantiles, estimated_cdf small → membership_score high → likely member
        '''

        return membership_score.cpu().numpy()

    # ==========================================================
    # EVALUATION
    # ==========================================================

    def evaluate_attack(self, attack_model,
                        member_logits, member_labels,
                        nonmember_logits, nonmember_labels):

        print("\n[QMIA] Evaluating attack...")

        member_scores = self.compute_membership_score(
            attack_model,
            member_logits,
            member_labels
        )

        nonmember_scores = self.compute_membership_score(
            attack_model,
            nonmember_logits,
            nonmember_labels
        )
        ...
        fpr, tpr, _ = roc_curve(true_labels, attack_scores)
...
class QMIARunner:
    ...
    def run(self, use_real_data=False, train_csv=None, test_csv=None):
        ...
        attack_model = attacker.train_attack_model(
            attack_model,
            nonmember_logits,
            nonmember_labels
        )
    ...
    def print_results(self, results):
        ...
        print(f"AUC: {results['attack_auc']:.6f}")

        member_scores = results["member_scores"]
        nonmember_scores = results["nonmember_scores"]

        print("\nMembership Score Statistics:")
        print(f"Member Score  - Mean: {member_scores.mean():.6f} ± {member_scores.std():.6f}")
        print(f"NonMember Score - Mean: {nonmember_scores.mean():.6f} ± {nonmember_scores.std():.6f}")
    ...
    def save_results(self, results, attack_model):
        ...
        metrics = {
            ...
            'member_score_mean': float(results['member_scores'].mean()),
            'member_score_std': float(results['member_scores'].std()),
            'nonmember_score_mean': float(results['nonmember_scores'].mean()),
            'nonmember_score_std': float(results['nonmember_scores'].std()),
            'attack_auc': float(results['attack_auc']),
            'num_quantiles': self.config.num_quantiles,
            'member_samples': len(results['member_scores']),
            'nonmember_samples': len(results['nonmember_scores'])
        }
    ...
    def plot_score_distributions(self, results):
        """Plot member vs non-member score distributions"""
        plt.figure(figsize=(12, 5))
        
        # Score distributions
        plt.subplot(1, 2, 1)
        plt.hist(results['member_scores'], bins=30, alpha=0.6, label='Members', color='red')
        plt.hist(results['nonmember_scores'], bins=30, alpha=0.6, label='Non-members', color='blue')
        plt.xlabel('Membership Score', fontsize=11)
        plt.ylabel('Count', fontsize=11)
        plt.title('Member vs Non-member Score Distribution', fontsize=12, fontweight='bold')
        plt.legend()
        plt.grid(True, alpha=0.3)
        
        # Box plot
        plt.subplot(1, 2, 2)
        data_to_plot = [results['member_scores'], results['nonmember_scores']]
        plt.boxplot(data_to_plot, labels=['Members', 'Non-members'])
        plt.ylabel('Membership Score', fontsize=11)
        plt.title('Score Distribution Comparison', fontsize=12, fontweight='bold')
        plt.grid(True, alpha=0.3, axis='y')
        
        plt.tight_layout()
        dist_path = os.path.join(self.config.results_dir, 'qmia_score_distributions.png')
        plt.savefig(dist_path, dpi=300, bbox_inches='tight')
        print(f"[QMIA] Score distributions saved to {dist_path}")
        plt.close()
...
def main():
    ...
    runner.plot_score_distributions(results)
